...
class RouteLoggerMiddleware(BaseHTTPMiddleware):
    def __init__(
        ...
        *,
        ...
        skip_routes: typing.Optional[typing.List[str]] = None,
        skip_regexes: typing.Optional[typing.List[str]] = None,
    ):
        self._logger = logger if logger else logging.getLogger(__name__)
        self._skip_routes = skip_routes if skip_routes is not None else []
        self._skip_regexes = (
    ...
    def _generate_success_log(
        ...
        request_log = f"Request {overall_status}, {request.method} {request.url.path}"
        status_code_log = f"status code={response.status_code}"
        execution_time_log = f"took={execution_time:0.4f}s"
        return f"{request_log}, {status_code_log}, {execution_time_log}"

    async def _execute_request(self, call_next: Callable, request: Request) -> Response:
        try:
            response = await call_next(request)
        except Exception:
            err = f"{request.url.path}, method={request.method}"
            self._logger.exception(
                f"Request failed with exception {err}",
            )
            raise
        return response
